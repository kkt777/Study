...
SELECT * FROM order_header WHERE total >= 5000 INTERSECT SELECT * FROM order_header WHERE total < 10000;

-- 4. 「関東」エリアの全店舗情報（店舗所在地と店舗情報）を抽出して下さい。※テーブル結合すること
SELECT * FROM area JOIN shop ON area.areacode = shop.areacode WHERE area.areacode = '04';
...
SELECT shop.shopcode, shop.shopname, area.areaname FROM shop JOIN area ON shop.areacode = area.areacode;

-- 8. 「新宿」店の在庫数が10以上の商品の商品コード（GoodsCode）、商品名（GoodsName）、在庫数（quantity）を抽出して下さい。
SELECT goods.goodscode, goods.goodsname, stock.quantity 
FROM stock JOIN goods ON stock.goodscode = goods.goodscode JOIN shop ON stock.shopcode = shop.shopcode 
WHERE shop.shopname = '新宿' AND stock.quantity >= 10;
